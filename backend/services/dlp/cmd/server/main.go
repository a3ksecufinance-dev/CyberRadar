...
import (
	...
	"github.com/cyberradar/platform/services/dlp/internal/handler"
...
func main() {
	...
	logger := log.With().Str("service", "dlp-service").Logger()
	...
	shutdownTracing, tracingErr := observe.InitTracing(context.Background(),
		"dlp-service", envOrDefault("SERVICE_VERSION", "dev"),
	...
	port := envOrDefault("SERVICE_PORT", "8021")
	...
	// ── Kafka producer (publishes DLP violation events) ───────────────────────
	producer := pkgkafka.NewProducer(pkgkafka.ProducerConfig{
		Brokers: brokers,
		Topic:   "crp.events.dlp",
	}, logger)

	// ── Repositories / services ───────────────────────────────────────────────
	dlpRepo := repository.NewDLPRepository(pool)
	dlpSvc := service.NewDLPService(dlpRepo, producer, logger)
	dlpH := handler.NewDLPHandler(dlpSvc)

	// ── HTTP server ───────────────────────────────────────────────────────────
	r := chi.NewRouter()
	r.Use(observe.Middleware("dlp-service"))
	...
	r.Get("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, `{"status":"ok","service":"dlp-service"}`)
	})
	...
	go func() {
		logger.Info().Str("addr", srv.Addr).Msg("dlp-service started")
	...
	logger.Info().Msg("dlp-service stopped")
}
