...
import (
	...
	"github.com/cyberradar/platform/services/easm/internal/handler"
...
func main() {
	...
	logger := log.With().Str("service", "easm-service").Logger()
	...
	shutdownTracing, tracingErr := observe.InitTracing(context.Background(),
		"easm-service", envOrDefault("SERVICE_VERSION", "dev"),
	...
	port := envOrDefault("SERVICE_PORT", "8019")
	...
	// ── Kafka producer (publishes EASM events) ────────────────────────────────
	producer := pkgkafka.NewProducer(pkgkafka.ProducerConfig{
		Brokers: brokers,
		Topic:   "crp.events.easm",
	}, logger)

	// ── Repositories / services ───────────────────────────────────────────────
	easmRepo := repository.NewEASMRepository(pool)
	easmSvc := service.NewEASMService(easmRepo, producer, logger)
	easmH := handler.NewEASMHandler(easmSvc)

	// ── HTTP server ───────────────────────────────────────────────────────────
	r := chi.NewRouter()
	r.Use(observe.Middleware("easm-service"))
	...
	r.Get("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, `{"status":"ok","service":"easm-service"}`)
	})

	r.Route("/api/v1", func(r chi.Router) {
		r.Use(authmw.RequireJWT(jwtVerifier, logger))
		r.Use(authmw.RequirePermissionByMethod("easm"))
		easmH.RegisterRoutes(r)
	})
	...
	go func() {
		logger.Info().Str("addr", srv.Addr).Msg("easm-service started")
	...
	logger.Info().Msg("easm-service stopped")
}
