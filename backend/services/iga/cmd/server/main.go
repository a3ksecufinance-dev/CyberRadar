...
import (
	...
	"github.com/cyberradar/platform/services/iga/internal/handler"
...
func main() {
	...
	shutdownTracing, tracingErr := observe.InitTracing(context.Background(),
		"iga-service", envOrDefault("SERVICE_VERSION", "dev"),
	...
	// ── Kafka producer (publishes IGA events) ─────────────────────────────────
	producer := pkgkafka.NewProducer(pkgkafka.ProducerConfig{
		Brokers: brokers,
		Topic:   "crp.events.iga",
	}, logger)
	...
	// ── HTTP server ───────────────────────────────────────────────────────────
	r := chi.NewRouter()
	r.Use(observe.Middleware("iga-service"))
	...
	r.Get("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, `{"status":"ok","service":"iga-service"}`)
	})

	r.Route("/api/v1", func(r chi.Router) {
		r.Use(authmw.RequireJWT(jwtVerifier, logger))
		r.Use(authmw.RequirePermissionByMethod("iga"))
		igaH.RegisterRoutes(r)
	})
	...
	go func() {
		logger.Info().Str("addr", srv.Addr).Msg("iga-service started")
	...
	logger.Info().Msg("iga-service stopped")
}
