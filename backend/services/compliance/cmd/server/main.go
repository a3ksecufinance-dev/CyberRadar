...
import (
	...
	"github.com/cyberradar/platform/services/compliance/internal/handler"
...
func main() {
	...
	logger := log.With().Str("service", "compliance-service").Logger()
	...
	shutdownTracing, tracingErr := observe.InitTracing(context.Background(),
		"compliance-service", envOrDefault("SERVICE_VERSION", "dev"),
	...
	port := envOrDefault("SERVICE_PORT", "8018")
	...
	compRepo := repository.NewComplianceRepository(pool)
	compSvc := service.NewComplianceService(compRepo, logger)
	compH := handler.NewComplianceHandler(compSvc)

	// ── Kafka producer (publishes compliance events) ───────────────────────────
	_ = pkgkafka.NewProducer(pkgkafka.ProducerConfig{
		Brokers: brokers,
		Topic:   "crp.events.compliance",
	}, logger)

	// ── HTTP server ───────────────────────────────────────────────────────────
	r := chi.NewRouter()
	r.Use(observe.Middleware("compliance-service"))
	...
	r.Get("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintf(w, `{"status":"ok","service":"compliance-service"}`)
	})

	r.Route("/api/v1", func(r chi.Router) {
		r.Use(authmw.RequireJWT(jwtVerifier, logger))
		r.Use(authmw.RequirePermissionByMethod("compliance"))
		compH.RegisterRoutes(r)
	})
	...
	go func() {
		logger.Info().Str("addr", srv.Addr).Msg("compliance-service started")
	...
	logger.Info().Msg("compliance-service stopped")
}
